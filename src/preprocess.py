...
target = "subtype"
sex = final_df["sex"]
ids = final_df["sample_id"]
X = final_df.drop(axis=1, labels=extra_data_df.columns, errors="ignore")
y = final_df[target]
X_train, X_test, y_train, y_test, sex_train, sex_test, ids_train, ids_test = train_test_split(X, y, sex, ids, stratify=y, test_size=0.3, random_state=1337)

pd.DataFrame(ids_train).to_csv(f"{save_prefix}/train_ids.csv", header=False, index=False)
...
for gene in tqdm(set(combined_df.columns) - set(["sample_id"])):
